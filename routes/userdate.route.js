...
.get(async (req, res) => {
	try{
		...
		const userDetails = await User.findById({_id: userId}).populate('likedVideos').populate('history').populate('watchLater').populate('playlist.list');
		const { likedVideos, history, watchLater, playlist, allNotes } = userDetails;
		res.json({ success: true, likedVideos, history, watchLater, playlist, allNotes });
	}catch (err){
...
.post(async (req, res) => {
	try{
		...
		await User.findByIdAndUpdate({_id: userId}, {likedVideos: userDetails.likedVideos });
...
.delete(async (req, res) => {
	try{
		...
		await User.findByIdAndUpdate({_id: userId}, {likedVideos: updatedList });
...
.post(async (req, res) => {
	try{
		...
		await User.findByIdAndUpdate({_id: userId}, {watchLater: userDetails.watchLater })
...
.delete(async (req, res) => {
	try{
		...
		await User.findByIdAndUpdate({_id: userId}, {watchLater: updatedList });
		res.json({ success: true, updatedList });
	}catch(err){
		console.log(err);
	}
...
.post(async (req, res) => {
	try{
		...
		if(findList){
			...
			updatedList = playlist.map(item => item.name === name ? item = findList : item);
		}else{
...
.post(async (req, res) => {
	try{
		...
		let filteredList = [];
		if(videoId){
			const findPlaylist = findUser.playlist.find(playlist => playlist._id == playlistId);
			const updatedList = findPlaylist.list.filter(id => id != videoId);
			filteredList = findUser.playlist.map(item => item._id == playlistId ? {...item._doc, list: updatedList} : item);
		}else{
			filteredList = findUser.playlist.filter(playlist => playlist._id != playlistId);
		}
...
.post(async (req, res) => {
		...
		if(findNotes === undefined){
			...
			updatedList = allNotes.map(item => item._id == videoId ? item = findNotes : item);
		}
